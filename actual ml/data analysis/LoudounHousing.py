...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 21 12:23:28 2022
...
def readDataFile(fileName):
    ...
    data = np.loadtxt(raw_data, usecols = (0,1,2, 6), skiprows = 1, delimiter=",")
...
def gradDesc(x, weights, y):
    predvals = np.dot(x.reshape(len(x),4),weights.reshape(4,1))
...
x,y = readDataFile("LoudounHousingData_Train.csv")
...
weights = np.zeros((rows, cols)).reshape(4,1)
...
minDiff = .01
vectDiff = 1
print("inital weights: " + str(weights.reshape(1,4)))
...
while iterations < maxIter and vectDiff > minDiff:
    ...
    weights = weights - (LR*(gradients.reshape(4,1)))
...
print("final weights: " + str(weights.reshape(1,4)))
print(iterations)
...
#test data
x1,y1 = readDataFile("LoudounHousingData_Test.csv")
